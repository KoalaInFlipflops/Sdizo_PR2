...
void Graph::printList()
{
    ...
    for (int i = 0; i < size_nodes; i++)
    {
        cout << "("<< i << ") -> "; // numer wierzchołka
        p = this->list[i];
        while (p)
        {
            cout << "  " << p->value << "(waga: " << p->weight << "),";// jego sąsiedzi i wagi
            p = p->next;
        }
        cout << endl;
    }
...
void Graph::loadGraphFromFile(const char *filename, bool directed)
{
    ...
    else
    {
        ...
        if (temp <= 0)
        {
            cout << "Liczba krawedzi jest nieprawidlowa" << endl;
            deleteStructure();
            return;
        }
        ...
        if (temp <= 0)
        {
            cout << "Liczba wierzcholkow jest nieprawidlowa" << endl;
            deleteStructure();
            return;
        }
        ...
        if (temp < 0)
        {
            cout << "Wierzcholek poczatkowy jest nieprawidlowy" << endl;
            deleteStructure();
            return;
        }
        ...
        for (int i = 0; i < this->size_edges; i++)
        {
            if (!(file >> from))
            {
                cout << "Rozmiar podany w pliku jest nieprawidlowy dla liczby elementow" << endl;
                deleteStructure();
                return;
            }
            if (!(file >> to))
